...
class Decoder(nn.Module):
  ...
  def forward(self, 
    ...
    # x: (BATCH, 1, INPUT_SIZE + HIDDEN_SIZE)
...
class PointerNetwork(nn.Module):
  ...
  def forward(self, 
    ...
    # x: (BATCH_SIZE, ARRAY_LEN, INPUT_LEN)
    ...
    # dec_in: (BATCH, 1, INPUT_SIZE)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    dec_in = torch.zeros(out.size(0), 1, self.input_size, dtype=torch.float).to(device)
    ...
    for t in range(out.size(1)):
      ...
      dec_in = torch.stack([x[b, int(idx[b].item())] for b in range(x.size(0))])
      dec_in = dec_in.view(out.size(0), 1, self.input_size).type(torch.float)
